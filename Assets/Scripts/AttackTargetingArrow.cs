...
/// arrowhead) as Transforms. Each frame this component moves them onto the ends of the curve, optionally
...
public class AttackTargetingArrow : MonoBehaviour
{
    ...
    [Tooltip("Arc height grows with attacker->target distance up to this cap (world units of extra bulge).")]
    public float maxExtraArc = 1.5f;

    [Header("Line width (taper from attacker to head)")]
    ...
    void LateUpdate()
    {
        ...
        Vector3 end = Cursor.instance != null ? (Vector3)Cursor.instance.mouseWorldPos : start;
