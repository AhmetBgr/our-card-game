...
public struct Cell
{
    ...
    public Cell(Vector3 position, Vector2Int index)
    {
        pos = position;
        this.index = index;
        obj = null;
...
public class GridManager : MonoBehaviour
{
    ...
    private void PopulateGrid()
    {
        ...
        Vector3 gridCenterOffset = new Vector3(1, 1, 0);

        for (int x = 0; x < GridWidth ; x++)
        {
            for (int y = 0; y < GridHeight; y++)
            {
                Vector3 cellPosition = new Vector3(x * cellSize , -y * cellSize, 0)/* + gridCenterOffset/*
                    + new Vector3(cellSize / 2.0f, cellSize / 2.0f, 0)*/;
                grid[x, y] = new Cell(cellPosition, new Vector2Int(x,y)); // Initialize the Cell with an empty list
    ...
    public void AddCellObjectToCell(int x, int y, GameObject obj)
    {
        if (IsOutSideOfGrid(new Vector2Int(x, y))) return;

        grid[x, y].cellObj = obj;  // Add a new object to the cell's list
    }
    ...
    public void RemoveCellObjectFromCell(int x, int y, GameObject obj)
    {
        if (grid[x, y].obj != null)
            grid[x, y].obj = null;  // Remove a specific object from the cell
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        for (int x = 0; x < GridWidth; x++)
        {
            ...
            {
                // Draw cell obj
                Gizmos.color = grid[x, y].cellObj != null ? Color.magenta : (Color.gray * new Color(1, 1, 1, 0.5f));
